...
def args_parser():
    ...
    parser.add_argument('--num_heads_HANLayer', type=int, default=8, help='')
    ...
    parser.add_argument('--layers_num_M', type=int, default=5, help='')
    parser.add_argument('--percent_selected_M', type=float, default=0.66, help='')
    parser.add_argument('--memory_num', type=int, default=5, help='')




    parser.add_argument('--scalability_time', type=bool, default=False, help='')
    parser.add_argument('--scalability_node', type=bool, default=False, help='')
    parser.add_argument('--scalability_num', type=int, default=0, help='')
...
def create_metapath_amazon(hg:dgl.DGLGraph, edge, pathnum=3):
    ...
    if etype == 0:
        ...
        metapath_dst1, _ = dgl.sampling.random_walk(hg, [dstnode] * pathnum,
                                                 metapath=['re-votes', 'votes', 're-votes', 'votes'])
...
def calucate_emb(X_in_dim,M_dim,inner_dim,X,M):
    ...
    to_K = nn.Linear(X_in_dim, M_dim, bias=False).to(M)
    out_ebs = torch.matmul(sim, M)

    K = torch.concatenate((to_K(X), out_ebs), dim=1)
    K = nn.Linear(M_dim*2, M_dim, bias=False).to(M)(K)
